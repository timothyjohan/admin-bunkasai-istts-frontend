// Komponen ini digunakan untuk menampilkan detail pendaftaran Lomba Yonkoma.
...
export default function YonkomaDetails() {
    // Mendefinisikan state lokal dengan hook `useState`
    const [yonkomaData, setYonkomaData] = useState({});
    ...
    // Mendefinisikan fungsi asinkron `getYonkomaDetails` untuk mengambil data dari API
    const getYonkomaDetails = async () => {
        try {
            const request = await axios.get(
                `${import.meta.env.VITE_API_URL}/api/yonkoma/${telp}`,
                {
                    headers: {
                        "x-auth-token": user,
                    },
            ...
            setYonkomaData(request.data);
            setStatus(request.data.status);

            // Mengambil bukti transfer menggunakan email dari data yonkoma
            if (request.data.email) {
                getBuktiTransfer(request.data.email);
            }
        } catch (error) {
            console.error("Gagal mengambil detail Yonkoma:", error);
        }
    };
    ...
    const getBuktiTransfer = async (email) => {
        try {
            const request = await axios.get(
                `${import.meta.env.VITE_API_URL}/api/transfer-proof/getTransferProof/${email}/yonkoma`,
                {
                    headers: {
                        "x-auth-token": user,
                    },
    ...
    // Menggunakan hook `useEffect` untuk memanggil `getYonkomaDetails` saat komponen dimuat
    useEffect(() => {
        if (user && telp) {
            getYonkomaDetails();
        }
